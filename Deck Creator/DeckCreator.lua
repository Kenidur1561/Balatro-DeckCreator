...
function DeckCreator.Enable(dkc)
        ...
        elseif noSpectralJokerKill then
            if self.ability.name == 'Ankh' then
                ...
                G.E_MANAGER:add_event(Event({trigger = 'before', delay = 0.4, func = function()
                    local card = copy_card(chosen_joker, nil, nil, nil, chosen_joker.p_edition and chosen_joker.p_edition.negative)
            ...
            elseif self.ability.name == 'Hex' then
                local temp_pool =   (self.ability.name == 'The Wheel of Fortune' and self.eligible_strength_jokers) or
                        ((self.ability.name == 'Ectoplasm' or self.ability.name == 'Hex') and self.eligible_p_editionless_jokers) or {}
                G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.4, func = function()
    ...
    function Card:open()
        ...
        local isSkipBlindOptionsEnabled = G.GAME and G.GAME.selected_back and G.GAME.selected_back.effect and G.GAME.selected_back.effect.config and G.GAME.selected_back.effect.config.customDeck and (G.GAME.selected_back.effect.config.spectral_cards_in_arcana or G.GAME.selected_back.effect.config.always_telescoping or G.GAME.selected_back.effect.config.never_telescoping or G.GAME.selected_back.effect.config.tarot_cards_in_spectral or G.GAME.selected_back.effect.config.tarot_cards_in_celestial or G.GAME.selected_back.effect.config.planet_cards_in_arcana or G.GAME.selected_back.effect.config.planet_cards_in_spectral or G.GAME.selected_back.effect.config.spectral_cards_in_celestial
                ...
                or G.GAME.selected_back.effect.config.extra_booster_pack_choices > 0 or (G.GAME.selected_back.effect.config.standard_pack_p_edition_rate and G.GAME.selected_back.effect.config.standard_pack_p_edition_rate ~= 2) or (G.GAME.selected_back.effect.config.standard_pack_enhancement_rate and G.GAME.selected_back.effect.config.standard_pack_enhancement_rate ~= 40) or (G.GAME.selected_back.effect.config.standard_pack_seal_rate and G.GAME.selected_back.effect.config.standard_pack_seal_rate ~= 20)) or false
    ...
    G.FUNCS.evaluate_round = function()
        ...
        if deck and deck.effect.config.customDeck then
            ...
            if deck.effect.config.negative_joker_money and deck.effect.config.negative_joker_money ~= 0 then
                ...
                for k,v in pairs(G.jokers.cards) do
                    if v.p_edition and v.p_edition.negative then
    ...
    function Card:set_cost()
        ...
        if self.p_edition then
            self.extra_cost = self.extra_cost + (self.p_edition.holo and 3 or 0) + (self.p_edition.foil and 2 or 0) +
                    (self.p_edition.polychrome and 5 or 0) + (self.p_edition.negative and 5 or 0)
        end
    ...
    function Card:click()
        if GUI.DeckCreatorOpen then
            ...
            if compareProto then
                local removeIndex
                for k,v in pairs(Utils.getCurrentEditingDeck().config.customCardList) do
                    if v.value == compareProto.value and v.suit == compareProto.suit and v.p_edition == compareProto.p_edition and v.enhancement == compareProto.enhancement and v.seal == compareProto.seal then
                        removeIndex = k
                        break
                    end
                end
        ...
        elseif GUI.StartingItemsOpen then
            ...
            if GUI.OpenStartingItemConfig.openItemType ~= nil then
                ...
                elseif GUI.OpenStartingItemConfig.openItemType == 'joker' then
                    added = CardUtils.addItemToDeck({ joker = true, ref = 'customJokerList', addCard = { id = self.config.center.key, key = self.config.center.key, copies = GUI.OpenStartingItemConfig.copies, eternal = GUI.OpenStartingItemConfig.eternal, pinned = GUI.OpenStartingItemConfig.pinned, p_edition = GUI.OpenStartingItemConfig.p_edition, perishable = GUI.OpenStartingItemConfig.perishable, rental = GUI.OpenStartingItemConfig.rental } })
                elseif GUI.OpenStartingItemConfig.openItemType == 'tarot' then
                    added = CardUtils.addItemToDeck({ tarot = true, ref = 'customTarotList', addCard = { key = self.config.center.key, copies = GUI.OpenStartingItemConfig.copies, p_edition = GUI.OpenStartingItemConfig.p_edition }})
                elseif GUI.OpenStartingItemConfig.openItemType == 'planet' then
                    added = CardUtils.addItemToDeck({ planet = true, ref = 'customPlanetList', addCard = { key = self.config.center.key, copies = GUI.OpenStartingItemConfig.copies, p_edition = GUI.OpenStartingItemConfig.p_edition }})
                elseif GUI.OpenStartingItemConfig.openItemType == 'spectral' then
                    added = CardUtils.addItemToDeck({ spectral = true, ref = 'customSpectralList', addCard = { key = self.config.center.key, copies = GUI.OpenStartingItemConfig.copies, p_edition = GUI.OpenStartingItemConfig.p_edition }})
                end
    ...
    function Back:apply_to_run()
        ...
        if self.effect.config.customDeck then
            ...
            G.GAME.p_edition_rate = config.p_edition_rate
    ...
    function Game:start_run(args)
        ...
        if deck and deck.effect and deck.effect.config and deck.effect.config.customDeck and not saveTable then
            ...
            for k,v in pairs(deck.effect.config.customTarotList) do
                table.insert(args.challenge.consumables, {id = v.key, p_edition = v.p_edition})
            end
        ...
        if deck.effect.config.customDeck then
            ...
            if not saveTable then
                for k, v in ipairs(args.challenge.consumables) do
                    G.E_MANAGER:add_event(Event({
                        func = function()
                            add_joker(v.id, v.p_edition, k ~= 1)
                            return true
                        end
                    }))
                end
            ...
            if deck.effect.config.custom_cards_set then
                ...
                local p_edition = config.random_p_edition_cards
                ...
                if randomizeRanks or randomizeSuits or noNumbered or noAces or poly > 0 or holo > 0 or foil > 0 or p_edition > 0 or bonus > 0 or glass > 0 or lucky > 0 or steel > 0 or stone > 0 or wild > 0 or mult > 0 or enhance > 0 then
